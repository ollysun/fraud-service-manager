	...
	public ServiceRule mapRuleToService(MapRuleToServiceRequest request) {
		...
		for(String name: datasetList) {
			if (ruleEntityOptional.get().getDataSourceValOne().equalsIgnoreCase("Transactional")
				...
				throw new FraudEngineException("The first source value: "  + ruleEntityOptional.get().getSourceValueOne() + "  not found in the dataset fieldName on ruleName : " + ruleEntityOptional.get().getName());
			}
			if ((StringUtils.isNotBlank(ruleEntityOptional.get().getDataSourceValTwo()) && ruleEntityOptional.get().getDataSourceValTwo().equalsIgnoreCase("Transactional")) &&
				...
				throw new FraudEngineException("The Second source value: "  + ruleEntityOptional.get().getSourceValueTwo() + "   not found in the dataset fieldName on ruleName : " + ruleEntityOptional.get().getName());
			}
		}
